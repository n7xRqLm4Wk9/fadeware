local loadstring = function(...)
	...
	if err and vape then vape:CreateNotification('FadeWare', 'Failed to load : '..err, 30, 'alert') end
	return res
end
...
local vm = loadstring(downloadFile('fadeware/libraries/vm.lua'), 'vm')()
...
run(function()
	...
	function jb:FireServer(id, ...)
		if not remotes[id] then
			notif('FadeWare', 'Failed to find remote ('..id..')', 10, 'alert')
			return
		end
