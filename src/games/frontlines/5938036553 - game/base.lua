...
local drawingactor = loadstring(downloadFile('fadeware/libraries/drawing.lua'), 'drawing')(...)
...
if not select(1, ...) and game.PlaceId == 5938036553 then
	if run_on_actor and getactors then
		...
		task.spawn(function()
			...
			local executionString = "loadfile('fadeware/main.lua')("..drawingactor..")"
			...
			notif('FadeWare', 'Failed to find actor', 10, 'alert')
		end)
	else
		vape.Load = function()
			notif('FadeWare', 'Missing actor functions.', 10, 'alert')
		end
	end
...
local function hookEvent(id, rfunc)
	...
	if not suc then
		notif('FadeWare', 'Failed to hook ('..id..')', 10, 'alert')
	end
